...
function Draw_MPC_track (t,xx,xx1,u_cl,x_ref,N,rob_diam,conesx,conesy,cones_diam,cone_horizon)
% Displays the results of the simulation for track based events
...
for k = 1:size(xx,2)
    ...
    if a>= length(x_ref)
        a= length(x_ref);
        cone_sample = length(conesx)-cone_horizon ;
    end
    ...
    fill(x1_tri, y1_tri, 'r'); % plot car position  
    ...
    title(["Track Simulation for MPC Controller","Velocity of Car: ", num2str(vx)], 'interpreter', 'latex' );
    axis([-5 30 -5 30])
    box on;
    grid on 
    drawnow
...
% Plot the computed control actions and velocities with time
figure(2)
subplot(3,1,1)
stairs(t,u_cl(:,1),'k','linewidth',1.5); axis([0 t(end) -4000 4000])
...
subplot(3,1,3)
stairs(t,xx(4,1:end-1,1),'b','linewidth',1.5); axis([0 t(end) 0 8])
xlabel('time (seconds)','interpreter', 'latex')
