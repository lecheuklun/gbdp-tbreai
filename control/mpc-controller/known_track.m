...
% retrieve cone coordinates x dim
conesx=

% retrieve cone coordinates y dim
conesy=
...
cone_horizon = 10; % number of cones

x_ref = ;  %Retrieve reference car positions and velocities
...
R = zeros(2,2); R(1,1) = 0.001; R(2,2) = 55; % weighing matrices (controls)
...
t0 = 0;
x0 = ;    %retrieve initial condition 6x1

if x(4)<0.5
    %{
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ADDITIONAL IMPLEMENTATION REQUIRED FOR CAR
    
    Due to the formulation of the vehicle model and the problems with
    CasADi addressed in the section 5.4.1 of GDBP paper. The car does not
    operate from a standstill and will not work well at low speeds. It is
    advised that a work around for this is used rather than a reformulation
    of the vehicle model which may be more involved.
    
    Overide the MPC controller and fix the steering wheel to the last
    position and hold increase the speed until the controller exceeds
    0.5m/s. This could be done using a simple controller or just observing
    the current velocity at regular intervals
    
    It might be beneficial to run the MPC in the background and then dont apply
    the control actions it calculates. Fix the state velocity to 1
    m/s to ensure the solution can be found. This may work around issues
    involving the updating of the controller. 
...
while(reference_pos<=length(x_ref))
    ...
    while(trigger==0 && mpc_iter < 40*laps / h)
        ...
        u_cl= [u_cl ; u(1,:)];
        t(mpc_iter+1) = t0;
        [t0, x0, u0] = ;                      % Apply the control actions
        u0 = [u(2:size(u,1),:);u(size(u,1),:)];
        xx(:,mpc_iter+2) = x0;                % Store the new location of vehicle
        X0 = reshape(full(sol.x(1:6*(N+1)))',6,N+1)'; % get solution 
        X0 = [X0(2:end,:);X0(end,:)];        % Shift predicted trajectory to initialize the next step
        mpc_iter = mpc_iter + 1;
