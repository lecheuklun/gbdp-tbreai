...
if(numel(Slip) > 1)
    ...
    if max(abs(Slip))>2 %FY
        Slip_limit = abs(0.3*Slip(Ind_max)); % CLK: the *0.3 is to get section close to origin
    ...
    if max(abs(Slip))>2 %must be slip angle if true - FY
        ...
        %okay, so if F_fit is less than 0, it flips the slip; why?!?!
